...
if (MAX_DISPLAY_SPECIAL_PRODUCTS > 0 ) {
  ...
  if ($num_products_count) {
    ...
    while (!$specials->EOF) {
		...
		$products_name = '<a class="name" href="' . zen_href_link(zen_get_info_page($specials->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($specials->fields["master_categories_id"]) . '&products_id=' . $specials->fields['products_id']) . '">' . substr(strip_tags($specials->fields['products_name']), 0, 60) . '</a>';
		...
		$products_butt2 = '<a href="' . zen_href_link(FILENAME_SPECIALS, zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $specials->fields["products_id"]) . '">' . zen_image_button(BUTTON_IMAGE_ADD_TO_CART, BUTTON_IN_CART_ALT1) . '</a>';
