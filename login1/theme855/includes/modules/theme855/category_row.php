...
if ($num_categories > 0) {
  ...
  while (!$categories->EOF) {
    ...
    $list_box_contents[$row][$col] = array('params' => 'class="categoryListBoxContents item_'.$count.'"',
                                           'text' => '<a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new) . '">' . zen_image(DIR_WS_IMAGES . $categories->fields['categories_image'], $categories->fields['categories_name'], SUBCATEGORY_IMAGE_WIDTH, SUBCATEGORY_IMAGE_HEIGHT) . '<span>' . $categories->fields['categories_name'] . '</span></a>');
	$count ++;
    $col ++;
